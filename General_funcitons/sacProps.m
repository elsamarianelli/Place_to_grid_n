function stats =sacProps(sac, shape)
...
%Get id of closest peaks - note closest peak 1 will be centre (EM edit)
...
closestPeakDistFromCentre=(closestPeaks(2:end));
...
if strcmp(shape, 'hexagon')
   ...
   angles = [60, 120, 30, 90, 150];
elseif strcmp(shape, 'square') % looking at non-hexagonal gridness
   rotatedCorr=ones(4,1)*nan;
   angles = [ 90, 180, 45, 135];
end

for rotAmnt     =angles %Change to '0:6:174' to calculate full gridness range
...
elseif strcmp(shape, 'square') % looking at non-hexagonal gridness
    gridness= min(rotatedCorr([1, 2])) - max(rotatedCorr([3, 4])); %This one for quick verison
end
