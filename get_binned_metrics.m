...
for i = 1:3
    for j = 1:3
        ...
        try
            ...
            % only save others if there are enough peaks to try with
            % smooth and check for number of peaks using ...
            % Natan, Adi. "Fast 2D peak finder." MATLAB Central File Exchange. 
            % Available at https://www. mathworks. com/matlabcentral/fileexchange/37388-fast-2d-peak-finder (2021).
            % to deal with peaks (works a bit better than matlab islocalmax2)
            ...
            if numel(peaks)/2 > 5
                ...
                binned_metrics_nine(i,j).scale_h = NaN;
                binned_metrics_nine(i,j).stGrd_h_el = NaN;
                binned_metrics_nine(i,j).eccent = NaN;
                binned_metrics_nine(i,j).orient = NaN;
                binned_metrics_nine(i,j).xyScale = NaN;
                binned_metrics_nine(i,j).abScale = NaN;
                binned_metrics_nine(i,j).ellipicity = NaN;
            end
        catch
