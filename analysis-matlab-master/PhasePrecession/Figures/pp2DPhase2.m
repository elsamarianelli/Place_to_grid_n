...
subplot(1,4,1)
    ...
    title(['N = ',num2str(numel(in.inStruct))],'FontSize', 8);

    subplot(1,4,2)
    in.saturation = 'variance';
    spatialRotationalAverage(x,y,pha,[],in);
%     title('Phase variance map of average field, normalised to the unit circle','FontSize', 8);
    colorbar
    ...
    if in.plotColorWheel
        ...
        title('Colour wheel (Key to phase map)','FontSize', 8);
    end
...
%     title('Occupancy normalised phase x position rate map', 'FontSize', 8);
...
subplot(1,4,4)
    hold off
    if numel(pha)>10000
        ind = ceil(eps + (numel(pha)-eps).*rand(10000,1));
    else
        ind = 1:numel(pha);
    end
    
    plot([reg(ind); reg(ind)], [pha(ind); pha(ind)+2*pi],'k.', 'MarkerSize', 1);
    axis square; hold on;
    ...
    title(sprintf('Circular R = %4.3f | p-value < %4.3f',...
        regress_out.cor, regress_out.p_shuffled), 'FontSize', 8);
    set(gca, 'Xlim', [-1 1], 'YLim', [0 4*pi], 'Xtick', [-1 0 1],...
        'YTick', 0:pi:4*pi, 'XTickLabel', [-1 0 1], 'TickLength', [0.035 0],...
        'YTickLabel', {'0', 'pi', '2pi', '3pi', '4pi'}, 'LineWidth', 2, 'FontSize', 8);
