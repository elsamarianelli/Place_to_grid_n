function [inArray] = createInputArrayForSargoliniDataSet
% This function is associated with the construction of an input array
% necessary to run ppMain specifically for the Sargolini 2006 data set
% available from the moser website.
%
...
txtFile = 'E:\Science\MetaData\Moser\Sargolini Grid Cells.txt';

% First load relevant trial metadata
fid = fopen(txtFile,'r');
% C = textscan(fid, '%s %d %s %d %d %s %d %d %d %d %s %s %s %s %s');
C = textscan(fid, '%s %d %s %d %d %d %s %s %s %s');
