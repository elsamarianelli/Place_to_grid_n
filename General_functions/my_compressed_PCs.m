function cells = my_compressed_PCs(env, n_cells, xy_field)
...
% paramater optimised to reduce diring rate variability across
...
%       cells: A cell array where each cell contains a structure with the place field map (fmap).
    ...
    % Define the acceptable range for the mean firing rate
    ...
    % After finding the optimal alpha, regenerate the final set of place cells
