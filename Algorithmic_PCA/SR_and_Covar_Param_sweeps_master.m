...
for iter = 1:n_iterations
    subfolder = fullfile(output_dir, sprintf('iteration_%d', iter));
...
parfor iter = 1:n_iterations
    ...
    output_dir_local = fullfile(getenv('USERPROFILE'), 'Documents', 'data', ...
        ['data_' method_tag, '_', traj_tag, '_', density_tag, '_', size_tag]);
    subfolder = fullfile(output_dir_local, sprintf('iteration_%d', iter));
    ...
    for PC = 1:In_local.n_cells
        ...
        sac = xPearson(map);
        GC_metrics{PC} = struct('map', map, 'sac', sac);
    end
