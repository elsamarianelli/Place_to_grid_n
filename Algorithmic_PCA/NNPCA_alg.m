...
%  inputting PC firing directly instead of training SR matrix as done in dordek
...
%% [2] Generate grids - hierarchical structure
...
switch method
    ...
    case 'nnpca'
        ...
        for z=1:NumberOfPC %per PC
            ...
            % subtract spatial  the mean from every feature
            % meanInputMat = mean(r_saved,2);
            % InputMatFixed = r_saved(:,1:L) - repmat(meanInputMat,1,L);
            % 
            % EM subtract temporal mean from every time step 
            meanInputMat = mean(r_saved,1);
            InputMatFixed = r_saved(:,1:L) - repmat(meanInputMat,size(elec,1),1);
