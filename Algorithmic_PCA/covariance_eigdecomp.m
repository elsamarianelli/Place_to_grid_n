...
for idx = 2:length(bound_ctrl_list)    % change based on which parmeter is being varied
    ...
    for iter = 1:In.n_iterations
        ...
        % Initialize waitbar
        h = waitbar(0, 'Reprojecting Principal Components...');
        
        for PC = 1:In.NumberOfPC
            % Update progress bar
            ...
            [metrics.stGrd_h, metrics.expGrd_h, metrics.scale_h] = multiGridness(sac, 'hexagon', map, "plot"); 
