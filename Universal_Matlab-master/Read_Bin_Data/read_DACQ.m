function [ data, cut ] = read_DACQ( pathFlnm, vars, loadWave )
...
%
%  --- ARGS --- 
% Can provide 0, 1, 2 or 3 arguments:
% pathFlnm  conjoined path & filname to load or leave as empty to use 
%           guiChoose
%
% vars      variable structure defining key set of variables for data
%           processing must contain either all of the values below OR be
%           empty (in which case default values are used) OR just provide
%           the pathFlnm argument and leave all others abscent (default
%           will be used again). If provided must include the following:
%           vars.pos.maxSpeed - speeds > are considered to be jumpy(cm/s)
...
%
%           NB call 'defaultVars' to load a copy of vars with default 
%           values
%
% loadWave  Logical [0 or 1]. Default is '1' in which case spike waveforms
%           are loaded (slow and lots of RAM) or '0' in which case they
%           aren't read from disk.
%
%
% --- RETURNS ---
% data      standard tint strucutre (no longer compatable with tint gui)
%
% cut       cell array of cut files where they exist - empty if not.
...
%%% HOUSE KEEPING %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1)Deal with path and filename
if ~exist('pathFlnm', 'var')
    currentDir=cd;
...
% 2)Deal with variables that control data processsing
if ~exist('vars', 'var') || isempty(vars)
    vars=default_vars; %Load default_vars if not specified
end

% 3) Do we want to read in the waveforms or not?
if ~exist('loadWave', 'var')
    loadWave    =1; %defult is to read in
end
...
for t=1:length(tetrode_list)
    ...
    if exist([data.path data.flnm '.' num2str(tetrode_list(t))],'file')
        ...
        if loadWave %Load waveforms too
...
if ~isempty(data.pos.led_pos) %if pos is present process as normal
    ...
    fprintf([data.flnm '.pos is empty, proceeding anyway.\n']);
end
