...
gScl2Test           =20:5:80; %Grid scales to test in cm [20:10:80]
...
%Note where number of fields is not an integer I generate an integer
%probablistically to match the required number of fields over all
...
for nn              =1:nGScl2Test
    ...
    for kk              =1:nLpPerScale
        ...
        nFld            =(100./gScl).^2 * nFld2Gsc; %Number fields - not an int
        nFld            =round(nFld) + (rand(1)<rem(nFld,1));
        ...
        [ grid(nn,kk,:), pVal(nn,kk,:,:), gridFrmShuff(nn,kk,:,:,:)] ...
            =fs_masterAnalysis( pos, spkPos, binSize, nLpForShuf);
